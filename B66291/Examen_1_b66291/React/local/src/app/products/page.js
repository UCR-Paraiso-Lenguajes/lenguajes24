  ...
  const loadData = async () => {
    try {
      ...
      if (!newCategoryId.searchText.trim() && selectedIdsString.trim()) {
        response = await fetch(`https://localhost:7013/api/store/products/categories?idSearch=noText&idSearchCat=${selectedIdsString}`);
      } else if (newCategoryId.searchText.trim() && !selectedIdsString.trim()) {
        response = await fetch(`https://localhost:7013/api/store/products/categories?idSearch=${newCategoryId.searchText}&idSearchCat=0`);
      } else {
        response = await fetch(`https://localhost:7013/api/store/products/categories?idSearch=${newCategoryId.searchText}&idSearchCat=${selectedIdsString}`);
      }
      if (!response.ok) {
